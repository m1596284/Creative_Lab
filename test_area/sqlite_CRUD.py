...
# set root logger for if call by other module
log = get_logger()
...
class Database:
    ...
    def use_table(self, table_name):
        ...
        """
        log.debug(f"Use table: {table_name}")
        self.table = table_name

    def sql_do(self, sql, *params):
        """Do a specific sql command

        Args:
                sql (str): string in sql format
        """
        log.debug(f"SQL do: {sql}")
        self.conn.execute(sql, params)
        self.conn.commit()

    def create_table(self, table_name, table_dict):
        """Create table by dict

        Args:
                table_name (str): table name
                table_dict (dict): keyword(column name):value(column type)
